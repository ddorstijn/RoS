...
public void drawBackground() {
  ...
  if(level == 1){
    ...
    noFill();
    for( int j=0; j<10; j++) {
    stroke( 255*noise(j/1.0f),255-255*noise(j/1.0f),255);
    beginShape();
    float f1 = noise(j/10.0f)+0.5f;
    float f2 = noise(j/7.0f);  
    float f3 = noise(j/1.3f)+.5f;
    float f4 = noise(j/1.2f,frameCount/500.0f);
    for( int i=0; i<width+10; i+=10) {
      vertex( i, height/2+(100*f4)*sin(f1*TWO_PI*i/600 + 100*f2 - frameCount/(100.0f*f3)));
    }
    endShape();
    }  
    filter(blur);
